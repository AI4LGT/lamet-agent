...
PARAM_RULES = (
    ...
    Depends("scheme_scan", "model_average", physics="At a fixed tail range, false publishes the single LA/NLA and prior-width model selected from the sample-average data and applies that same model to every resample; true forms normalized exp(logGBF-max(logGBF)) weighted means over all numerically valid candidates, without Q filtering or an explicit between-model variance term."),
    Recommends("scheme_scan", "q_min", physics="Preferred lsqfit-Q threshold for sample-average range selection and the non-averaged center model choice; after the recommendation budget is exhausted, publication falls back to the numerically valid center model with maximum Q.", default=0.05),
    ...
    Value("scheme_scan.model_average", bool, physics="false publishes the window-selected LA/NLA and prior-width model for every resample; true forms per-resample evidence-weighted means over valid models, without Q filtering or a separate between-model variance term."),
